 ...
 */
	class UserApi  extends aplStdAJAXMethod
	{
		...
		protected function get_compensations_row_AJAX(){
			$this->responseClass->response['data'] = $this->get_all_tbl(COMPENSATION_TBL,array('user_id'=>(int)$_POST['user_id']));
		}
		...
		protected function save_date_work_start_AJAX(){
			$this->update_one_val_in_one_row(MANAGERS_TBL,$_POST['id'],'date_start_wock',date("Y-m-d",strtotime($_POST['date'])));
		}
		...
		private function get_all_tbl($table,$where = array(),$sort = array('name'=>'','type'=>"ASC")){
			$query = "SELECT *";
//			$query .= "DATE_FORMAT(`".$table."`.`date`,'%d.%m.%Y') as date ";
			$query .= " FROM `".$table."`";
			$w = 0;
